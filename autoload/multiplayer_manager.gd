...
func request_send_units(
	...
	if is_multiplayer():
		if multiplayer.is_server():
			_handle_send_units_request(source_id, target_id, use_percent, value, multiplayer.get_unique_id())
		else:
			request_send_units_rpc.rpc_id(1, source_id, target_id, use_percent, value)
	else:
		var source := get_structure(source_id)
		var target := get_structure(target_id)
		if source and target:
			source.send_units(target, use_percent, value)
...
func _handle_send_units_request(
	source_id: int,
	target_id: int,
	use_percent: bool,
	value: float,
	...
	if not peer_teams.has(peer_id):
		Globals.send_error("Unknown peer %d attempted to send units" % peer_id)
		return

	var source := get_structure(source_id)
	var target := get_structure(target_id)
	if source == null or target == null:
		Globals.send_error("Invalid structure ids: %d -> %d" % [source_id, target_id])
		return

	if source == target:
		return

	if source.get_team() != peer_teams[peer_id]:
		Globals.send_error("Peer %d does not own team %d" % [peer_id, source.get_team()])
		return
